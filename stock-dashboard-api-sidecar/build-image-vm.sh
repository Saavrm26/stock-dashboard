...
IMAGE_TAG="${IMAGE_TAG:-ghcr.io/saavrm26/stock-dashboard/sidecar:latest}"
...
log() {
  printf '[stock-dashboard-api-sidecar] %s\n' "$*"
}

die() {
  printf '[stock-dashboard-api-sidecar] Error: %s\n' "$*" >&2
  exit 1
}
...
Builds the stock-dashboard sidecar image on a Debian/Linux VM.

Environment overrides:
  IMAGE_TAG      Target image tag (default: ghcr.io/saavrm26/stock-dashboard/sidecar:latest)
...
Examples:
  ...
  IMAGE_TAG=ghcr.io/saavrm26/stock-dashboard/sidecar:dev ./build-image-vm.sh --install-docker
EOF
}
...
main() {
  ...
  ensure_base_image "docker.io/astral/uv:0.10.9"
  build_image
}
