...
class WatchListServiceV1Test {
    ...
    fun `createWatchList resolves missing catalog tickers from the sidecar`() {
        ...
        val response = watchListServiceV1.createWatchList(TEST_USER_ID, watchListMapper.toCreateWatchListServiceModel(request))
    ...
    @Test
    @Order(3)
    fun `createWatchList dedupes repeated ticker codes into a single catalog row`() {
        userRepository.save(User(id = TEST_USER_ID, email = TEST_USER_EMAIL, fullName = TEST_USER_NAME))
        ...
        assertNotNull(response.id)
        assertEquals(1, tickerRepository.count())
    }
    ...
    fun `createWatchList fails before any write when the sidecar omits a missing code`() {
        ...
        assertThrows(StockClientException::class.java) {
            watchListServiceV1.createWatchList(TEST_USER_ID, watchListMapper.toCreateWatchListServiceModel(request))
        }
