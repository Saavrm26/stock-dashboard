...
class WatchListMapperReverseMappingTest {
    ...
    fun `toDbWatchList maps service watchlist to db entity with provided user`() {
        ...
        val result = watchListMapper.toDbWatchList(serviceWatchList, serviceUser)

        assertEquals("Test Watchlist", result.name)
        assertEquals("Test description", result.description)
        assertEquals(serviceUser.id, result.createdBy?.id)
    ...
    @Test
    fun `toCreateWatchListServiceModel maps WatchListRequest to CreateWatchList`() {
        ...
        val result = watchListMapper.toCreateWatchListServiceModel(request)

        assertEquals("Tech Stocks", result.name)
        assertEquals("My favorite tech companies", result.description)
        assertEquals(WatchListVisibility.PUBLIC, result.visibility)
        assertEquals(WatchListType.FIXED, result.type)
        assertEquals("query", result.screenQuery)
        assertEquals(1, result.tickers.size)
        assertEquals("AAPL", result.tickers[0].tickerCode)
        assertEquals("NASDAQ", result.tickers[0].tickerExchange)
    }
}
