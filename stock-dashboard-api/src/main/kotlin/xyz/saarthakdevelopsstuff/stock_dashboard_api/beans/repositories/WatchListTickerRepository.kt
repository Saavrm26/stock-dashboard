...
interface WatchListTickerRepository : JpaRepository<WatchListTicker, Long> {
    ...
    @Modifying
    @Query("DELETE FROM WatchListTicker wlt WHERE wlt.watchList.id = :watchListId AND wlt.tickerCode IN :tickerCodes")
    fun deleteByWatchListIdAndTickerCodeIn(watchListId: Long, tickerCodes: List<String>): Int
}
