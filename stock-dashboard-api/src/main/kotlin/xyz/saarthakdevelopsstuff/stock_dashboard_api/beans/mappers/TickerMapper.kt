...
abstract class TickerMapper {
    ...
    fun toDbTicker(tickerDetails: TickerDetails): Ticker {
        return Ticker(
            tickerCode = tickerDetails.symbol,
            tickerLongName = tickerDetails.tickerName ?: tickerDetails.symbol,
            tickerExchange = tickerDetails.exchange.orEmpty(),
            tickerDetails = objectMapper.writeValueAsString(tickerDetails)
        )
    }

    fun toDbTickers(tickerDetailsList: List<TickerDetails>): List<Ticker> {
        return tickerDetailsList.map { toDbTicker(it) }
    }
